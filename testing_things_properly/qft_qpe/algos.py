...
def qdrift_sample(hamiltonian: SparsePauliOp, time: float, num_samples: int, N: int) -> Tuple[List[SparsePauliOp], List[str]]:
...
def qdrift_qpe(hamiltonian: SparsePauliOp, time: float, eigenstate, num_qubits: int, num_ancilla: int, num_samples: int):
    qc = QuantumCircuit(num_ancilla + num_qubits, num_ancilla)
    ...
    for k in range(num_ancilla):
        for _ in range(2 ** k):
            ...
            sampled_unitaries, labels = qdrift_sample(hamiltonian, time, num_samples=1, N=num_samples)
            for unitary, label in zip(sampled_unitaries, labels):
                controlled_unitary = UnitaryGate(unitary, label=label).control(1)
