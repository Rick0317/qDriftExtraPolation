...
from algos_optimized import prepare_eigenstate_circuit, make_pauli_gate_cache, build_template_circuit, build_qdrift_trajectory

from algos import generate_ising_hamiltonian
...
TIMES     = np.logspace(base=2, start=-10, stop=1, num=100, endpoint=True)
...
@cache
def _qdrift_factory(ham_key: str,
                    n_anc: int,
                    ground: bool
                    ) -> Callable[[np.random.Generator, float], QuantumCircuit]:
    """
    Build the STATIC artefacts once and return a closure that produces
    a fresh qDRIFT trajectory when called.

    Returned signature:
        make_traj(rng, total_time) -> QuantumCircuit

    """

    # ––– 1)  Heavy template built once ––––––––––––––––––––––––––––
    tmpl = template_circuit(ham_key, n_anc, ground)   # cached, but still O(size)

    # ––– 2)  Precompute where the PLACEHOLDER gates sit ––––––––––––
    placeholder_idx: list[int] = [
        i for i, instr in enumerate(tmpl.data)
        if getattr(instr.operation, "label", None) == PLACEHOLDER
    ]

    # ––– 3)  Fetch the small pool of PauliEvolutionGate objects ––––
    pauli_gates: Sequence[PauliEvolutionGate] = list(pauli_cache(ham_key).values())

    # ––– 4)  Define the closure ––––––––––––––––––––––––––––––––––––
    def _make(rng: np.random.Generator, total_time: float) -> QuantumCircuit:
        qc = tmpl.copy()                      # shallow copy, O(nodes)
        for idx in placeholder_idx:           # only 'segments' iterations
            gate = rng.choice(pauli_gates)
            qc.data[idx] = qc.data[idx]._replace(operation=gate)
        # assign the total time to the global τ parameter
        tau_param = pauli_gates[0].params[0]  # all gates
        qc = qc.assign_parameters({tau_param: total_time})
        # print(f"Built qDRIFT trajectory for {ham_key} with {n_anc} ancillas in process {os.getpid()}.")
        return qc
...
def run_simulation(experimental_conditions: dict[str, object]) -> QPEResult:
    ...
    # ─── fetch the fast circuit generator (closure) ──────────────
    make_traj = _qdrift_factory(ham_key, n_anc, ground_state)
