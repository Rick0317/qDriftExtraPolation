...
def qDrift_sample(hamiltonian: SparsePauliOp):
        ...
        h_j = SparsePauliOp([pauli_string], [original_coeff])
        return h_j


def generate_kitaev_qpe_circuit_qdrift(hamiltonian, M, eigen_circuit, theta):
    """
    ...
    for _ in range(M):
        ...
        evolution_op = PauliEvolutionGate(sampled_H, time=-1)
...
def get_qDrift_kitaev_result_v2(hamiltonian, eigen_circuit, m, qDrift_invocation, shots_per_estimation=1024):
    ...
    for j in range(1, m + 1):
        ...
        cos_val = 0
        for _ in range(qDrift_invocation):
            # Estimate cos(M * lambda) using qDrift circuits
            qc_cos = generate_kitaev_qpe_circuit_qdrift(hamiltonian, M, eigen_circuit, theta=0,)
            compiled_cos = transpile(qc_cos, simulator)
        ...
        cos_val /= qDrift_invocation
        sin_val = 0
        for _ in range(qDrift_invocation):
            # Estimate sin(M * lambda) using qDrift circuits
            qc_sin = generate_kitaev_qpe_circuit_qdrift(hamiltonian, M, eigen_circuit, theta=np.pi/2,)
            compiled_sin = transpile(qc_sin, simulator)
